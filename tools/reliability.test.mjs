...
test("complete algebra has no catalog prerequisite and has explained practice", () => {
  ...
  const sections = STUDY.MATH101.units.flatMap(u => u.sections); assert.equal(sections.length, 80);
...
function serviceWorker(fetcher, hit = new Response("<body>saved</body>"), base = "https://campus.test/"){
  ...
  const context = { self: { registration: { scope: base }, addEventListener: (k, f) => handlers[k] = f, clients: { claim: async () => {} }, location: { origin: "https://campus.test" } },
    caches: { keys: async () => ["pembroke-v150-shell", "pembroke-v152-shell", "pembroke-v153-shell", "pembroke-v154-shell", "pembroke-v156-shell", "pembroke-v157-shell", shell, "pembroke-assets-v3-depot", "other-app-cache"], delete: async k => removed.push(k), open: async () => cache },
...
test("activation deletes only obsolete Pembroke caches", async () => {
  ...
  assert.notEqual(sw.shell, "pembroke-v153-shell");
  assert.notEqual(sw.shell, "pembroke-v154-shell");
  assert.notEqual(sw.shell, "pembroke-v156-shell");
  assert.notEqual(sw.shell, "pembroke-v157-shell");
  assert.deepEqual(sw.removed, ["pembroke-v150-shell", "pembroke-v152-shell", "pembroke-v153-shell", "pembroke-v154-shell", "pembroke-v156-shell", "pembroke-v157-shell"]);
});
