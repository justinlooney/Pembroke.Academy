...
try{
  ...
  console.log('ok — all 73 pages and every required practice set render on a phone, preserving complete math text');
  ...
  for(let i=1;i<5;i++){const host=page.locator(`[data-problem="${i}"]`);await host.locator('input').fill(String(sec.full.practice[i].ans));await host.locator('[data-check]').click();}
  assert.doesNotMatch(await page.locator('#practice-status').innerText(),/practice cleared/);
  const last=page.locator('[data-problem="5"]');await last.locator('input').fill('4');await last.locator('[data-check]').click();
  ...
  if(process.env.PEMBROKE_CAPTURE_DIR){const dir=process.env.PEMBROKE_CAPTURE_DIR;await mkdir(dir,{recursive:true});await page.screenshot({path:resolve(dir,'algebra-conic-mobile.png')});await open('P.1');await page.locator('h1').scrollIntoViewIfNeeded();await page.screenshot({path:resolve(dir,'algebra-reading-mobile.png')});await page.setViewportSize({width:1440,height:960});await open('MX.2');await page.screenshot({path:resolve(dir,'algebra-reading-desktop.png')});}
