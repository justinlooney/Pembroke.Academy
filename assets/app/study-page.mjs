...
const setOf = (id, n) => id === "MATH201" ? MATH201_PSET[n] : sections(id).find(s => s.n === n)?.full?.practice || null;
...
function navigation(id, n){
  ...
  const nav = document.getElementById("lessons"), search = document.getElementById("lesson-search");
  if (search.dataset.course !== id){ search.value = ""; search.dataset.course = id; }
  search.closest(".contents-search").hidden = !plan;
  const query = search.value.trim().toLocaleLowerCase();
  const matches = plan?.units.map(u => ({...u, sections:u.sections.filter(s =>
    (u.title + " " + s.n + " " + s.t + " " + s.brief).toLocaleLowerCase().includes(query))})).filter(u => u.sections.length) || [];
  nav.innerHTML = matches.map(u => `<details class="chapter" ${query || u.sections.some(s => s.n === n) ? "open" : ""}><summary>${esc(u.title)} <small>${u.sections.length}</small></summary>${u.sections.map(s => `<a href="${route(id, s.n)}" ${s.n === n ? 'aria-current="page"' : ""}>${state[id]?.[s.n] === 2 ? "✓ " : ""}${esc(s.n)} · ${esc(s.t)}</a>`).join("")}</details>`).join("");
  document.getElementById("contents-status").textContent = query ? `${matches.reduce((sum,u)=>sum+u.sections.length,0)} lessons found` : "";
  search.oninput = () => navigation(id, n);
}
function inputs(q, name){
  return q.type === "mc" || q.opts ? q.opts.map((o, i) => `<label><input type="radio" name="${name}" value="${i}"><span>${esc(o)}</span></label>`).join("")
    : `<label>Your answer <input type="text" inputmode="text" name="${name}" autocomplete="off"></label>`;
}
...
function checkForm(id, sec){
  return `<h2 id="check-heading" tabindex="-1">Knowledge check</h2><p>Answer every question. Pass all checks${setOf(id, sec.n) ? " and earn at least 75% of the required problem set" : ""} to master this lesson.</p><form id="knowledge">${sec.qs.map((q, i) => `<fieldset data-question="${i}"><legend>${i + 1}. ${esc(q.q)}</legend>${inputs(q, "kc" + i)}<p class="feedback" role="status"></p></fieldset>`).join("")}<button>Check my work</button><p id="check-status" role="status"></p></form>`;
}
function turnForm(q, i){ return `<fieldset data-turn="${i}"><legend>${esc(q.q)}</legend>${inputs(q, "turn" + i)}<button type="button">Check answer</button> <button type="button" data-solution>Show solution</button><p class="feedback" role="status"></p></fieldset>`; }
function focusLesson(){ lesson.focus({preventScroll:true}); lesson.scrollIntoView({block:"start"}); }
function render(focus = false){
  ...
  if (catalog){
    ...
    mountCatalog(lesson, state); if (focus) focusLesson(); return;
  }
  ...
  lesson.innerHTML = `<div class="lesson-masthead"><p class="eyebrow">${esc(course.code)} · Lesson ${esc(n)}</p><h1>${sec.t}</h1><p class="lesson-lead">${fmt(sec.brief)}</p></div><nav class="lesson-stages" aria-label="Within this lesson"><button type="button" data-jump="reading-start">Read</button>${full?.viz ? '<button type="button" data-jump="explore-heading">Explore the model</button>' : ""}${full ? '<button type="button" data-jump="examples-heading">Worked examples</button>' : ""}<button type="button" data-jump="check-heading">Check your understanding</button></nav><p class="key" id="reading-start" tabindex="-1">${fmt(sec.key)}</p>
    ...
    ${full.viz ? `<h2 id="explore-heading" tabindex="-1">Explore the model</h2><canvas id="figure" width="640" height="360" role="img" aria-label="${esc(full.viz.note)}"></canvas><label for="model">Interactive control</label><input id="model" type="range" min="0" max="1000" value="0"><p id="readout" class="readout" aria-live="polite"></p><p>${fmt(full.viz.note)}</p>` : ""}
    <h2 id="examples-heading" tabindex="-1">Worked examples</h2>${[full.worked, ...(full.examples || [])].map((example,i) => `<section class="worked-example"><h3>Example ${i+1}</h3><p>${fmt(example.prompt)}</p>${example.steps.map(([ask,reveal],j)=>`<details><summary>${j+1}. ${fmt(ask)}</summary><p>${fmt(reveal)}</p></details>`).join("")}</section>`).join("")}<h2>Your turn</h2><p class="answer-help">Enter a number, a decimal, or a fraction such as −3/4.</p>${full.turn.map(turnForm).join("")}` : ""}
    ${checkForm(id, sec)}
    ...
    <div class="actions">${sections(id).indexOf(sec) > 0 ? `<a href="${route(id, sections(id)[sections(id).indexOf(sec)-1].n)}">← Previous lesson</a>` : ""}${sections(id)[sections(id).indexOf(sec) + 1] ? `<a href="${route(id, sections(id)[sections(id).indexOf(sec) + 1].n)}">Next lesson →</a>` : `<p>${id === 'MATH101' ? 'You have reached the end of College Algebra. Revisit any chapter or use the cumulative exam to guide your review.' : 'You have reached the end of the currently available lessons for this course.'}</p>`}</div>`;
  ...
  if (full){
    ...
    full.turn.forEach((q, i) => {
      ...
      host.querySelector("[data-solution]").onclick = () => { host.querySelector(".feedback").textContent = "Solution: " + q.work; };
...
function homework(id, sec){
  ...
  lesson.innerHTML = `<h1>${esc(sec.full.homework.title)}</h1><p>Enter numbers, decimals, or fractions. Submit your attempt to see explained solutions.</p><form>${qs.map((q, i) => `<fieldset data-hw="${i}"><legend>${esc(q.q)}</legend>${inputs(q, "hw" + i)}<p class="feedback" role="status"></p></fieldset>`).join("")}<button>Check homework</button><p id="hw-status" role="status"></p></form><button id="back">Back to lesson</button>`;
  lesson.querySelector("form").onsubmit = e => {
    ...
    results.forEach((r, i) => { feedback(hosts[i], r, "✓ " + (qs[i].work || qs[i].sol || "Correct"), "Solution: " + (qs[i].work || qs[i].sol || qs[i].ans)); recordCheck(state, id, n, "hw", r.correct); });
  ...
  lesson.querySelector("#back").onclick = () => render(true); focusLesson();
}
function practice(id, sec){
  ...
  const question = (q, key) => {
    questions.set(key, q);
    return `<fieldset data-problem="${key}"><legend>${esc(q.q)}</legend>${inputs(q, "ps" + key)}<div class="actions"><button type="button" data-check>Check answer</button><button type="button" data-hint>Hint</button></div><p class="feedback" role="status"></p></fieldset>`;
  };
